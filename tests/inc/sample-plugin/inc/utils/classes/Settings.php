...
use function ReallySpecific\SamplePlugin\Utils\Text\parsedown_line;
class Settings
{
    ...
    public function render_multicheck(array $field, $value, array $attrs)
    {
        ...
        foreach ($field['options'] as $key => $option) {
            ...
            foreach ($suboptions as $subkey => $suboption) {
                ...
                $buffer .= sprintf('<p class="rs-util-settings-field__option"><input type="%1$s" value="%2$s" %3$s><label for="%2$s">%4$s</label></p>', empty($attrs['multiple']) ? 'radio' : 'checkbox', esc_attr($subkey), array_to_attr_string($subattrs), esc_html($label));
            }
            if (isset($option['group'])) {
                $buffer .= '</fieldset>';
            }
