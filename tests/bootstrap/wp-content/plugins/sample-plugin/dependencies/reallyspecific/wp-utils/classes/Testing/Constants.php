...
class Constants
{
    ...
    public function add_constant(string $constant_name, $constant_value, $register = \true)
    {
        if (defined($constant_name)) {
            return;
        }
        $this->constants[$constant_name] = ['registered' => \false, 'value' => $constant_value];
        if ($register) {
            $this->register_constant($constant_name);
        }
    }
    ...
    {
        $constants = ['WP_CONTENT_DIR' => $this->mock->get_env('content_path'), 'ABSPATH' => $this->mock->get_env('root_path'), 'WPINC' => $this->mock->get_env('root_path') . '/wp-includes', 'EZSQL_VERSION' => 'WP1.25', 'OBJECT' => 'OBJECT', 'OBJECT_K' => 'OBJECT_K', 'ARRAY_A' => 'ARRAY_A', 'ARRAY_N' => 'ARRAY_N', 'WP_DEBUG' => \false];
