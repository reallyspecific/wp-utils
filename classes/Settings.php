		...
		switch( $tag ) {
			case 'input':
				...
				if ( $attrs['type'] === 'checkbox' || $attrs['type'] === 'radio' ) {
					$render_template .= '<svg xmlns="http://www.w3.org/2000/svg" class="rs-util-settings-field-icon"><use href="#rs-util-svg-iconset--boxicons--toggle" /></svg>';
				}
		...
		foreach( $field['options'] as $key => $option ) {
			...
			foreach( $suboptions as $subkey => $suboption ) {
				...
				$buffer .= sprintf( 
					...
					. '<svg xmlns="http://www.w3.org/2000/svg" class="rs-util-settings-field-icon"><use href="#rs-util-svg-iconset--boxicons--toggle" /></svg>'
